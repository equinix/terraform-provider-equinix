...
func specifiedVlanIds(d *schema.ResourceData) []string {
	...
	if vlanIdsOk {
		return converters.ConvertStringArr(vlanIdsRaw.(*schema.Set).List())
	}

	vxlanIdsRaw, vxlanIdsOk := d.GetOk("vxlan_ids")
	if vxlanIdsOk {
		return converters.ConvertIntArr(vxlanIdsRaw.(*schema.Set).List())
	}
	return []string{}
}
...
func batchVlans(removeOnly bool) func(*ClientPortResource) error {
	return func(cpr *ClientPortResource) error {
		...
		vlansToRemove := comparisons.Difference(
		...
		if !removeOnly {
			...
			vlansToAssign = comparisons.Difference(
				specifiedVlanIds(cpr.Resource),
				attachedVlanIds(cpr.Port),
			)
		}
...
func convertToL2(cpr *ClientPortResource) error {
	...
	isLayer2 := comparisons.Contains(l2Types, cpr.Port.NetworkType)
...
func portSanityChecks(cpr *ClientPortResource) error {
	...
	if nativeVlanOk {
		...
		if !comparisons.Contains(vlans, nativeVlan) {
