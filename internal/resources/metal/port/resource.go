...
func resourceMetalPortRead(d *schema.ResourceData, meta interface{}) error {
	...
	l2 := comparisons.Contains(l2Types, port.NetworkType)
	l3 := comparisons.Contains(l3Types, port.NetworkType)
...
func resourceMetalPortDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if resetOk && resetRaw.(bool) {
		...
		cpr.Resource = copy
		if err = maps.SetMap(cpr.Resource, map[string]interface{}{
			"layer2":         false,
			"bonded":         true,
			"native_vlan_id": nil,
			"vlan_ids":       []string{},
