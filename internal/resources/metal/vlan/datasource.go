...
func dataSourceMetalVlanRead(d *schema.ResourceData, meta interface{}) error {
	...
	if !(vlanIdOk || (vxlanOk || projectOk || metroOk || facilityOk)) {
		return errors.FriendlyError(fmt.Errorf("You must set either vlan_id or a combination of vxlan, project_id, and, metro or facility"))
	}
...
func matchingVlan(vlans []packngo.VirtualNetwork, vxlan int, projectID, facility, metro string) (*packngo.VirtualNetwork, error) {
	...
	if len(matches) > 1 {
		return nil, errors.FriendlyError(fmt.Errorf("Project %s has more than one matching VLAN", projectID))
	}

	if len(matches) == 0 {
		return nil, errors.FriendlyError(fmt.Errorf("Project %s does not have matching VLANs", projectID))
	}
