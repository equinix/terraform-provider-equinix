...
func resourceMetalVlanCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if !facOk && !metroOk {
		return eqerr.FriendlyError(errors.New("one of facility or metro must be configured"))
	}
	if facOk && vxlanOk {
		return eqerr.FriendlyError(errors.New("you can set vxlan only for metro vlans"))
	}
	...
	if err != nil {
		return eqerr.FriendlyError(err)
	}
	d.SetId(vlan.ID)
...
func resourceMetalVlanRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		err = eqerr.FriendlyError(err)
		if eqerr.IsNotFound(err) {
			d.SetId("")
			return nil
		}
		return err
...
func resourceMetalVlanDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if eqerr.IgnoreResponseErrors(eqerr.HttpForbidden, eqerr.HttpNotFound)(resp, err) != nil {
		return eqerr.FriendlyError(err)
	} else if err != nil {
	...
	return eqerr.FriendlyError(eqerr.IgnoreResponseErrors(eqerr.HttpForbidden, eqerr.HttpNotFound)(client.ProjectVirtualNetworks.Delete(id)))
}
