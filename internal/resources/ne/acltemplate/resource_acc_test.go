...
func testSweepNetworkACLTemplate(region string) error {
	...
	templates, err := config.NEClient.GetACLTemplates()
	if err != nil {
	...
	for _, template := range templates {
		if !tfacc.IsSweepableTestResource(ne.StringValue(template.Name)) {
			nonSweepableCount++
			continue
		}
		if err := config.NEClient.DeleteACLTemplate(ne.StringValue(template.UUID)); err != nil {
