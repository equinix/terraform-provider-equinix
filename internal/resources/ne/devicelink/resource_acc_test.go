...
func testSweepNetworkDeviceLink(region string) error {
	...
	links, err := config.NEClient.GetDeviceLinkGroups()
	if err != nil {
	...
	for _, link := range links {
		if !tfacc.IsSweepableTestResource(ne.StringValue(link.Name)) {
			nonSweepableCount++
			continue
		}
		if err := config.NEClient.DeleteDeviceLinkGroup(ne.StringValue(link.UUID)); err != nil {
...
func TestAccNetworkDeviceLink(t *testing.T) {
	...
	metroSecondary, _ := schema.EnvDefaultFunc(tfacc.NEDeviceSecondaryMetroEnvVar, metro)()
	...
	context := map[string]interface{}{
		...
		"link-name":                         fmt.Sprintf("%s-%s", tfacc.TestResourcePrefix, acctest.RandString(6)),
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: tfacc.NewTestAccConfig(context, withDevice, withDeviceLink).Build(),
...
func withDeviceLink(t *tfacc.TestAccConfig) {
	t.Use(testAccNetworkDeviceLink)
}
