...
func testSweepNetworkSSHKey(region string) error {
	...
	keys, err := config.NEClient.GetSSHPublicKeys()
	if err != nil {
	...
	for _, key := range keys {
		if !tfacc.IsSweepableTestResource(ne.StringValue(key.Name)) {
			nonSweepableCount++
			continue
		}
		if err := config.NEClient.DeleteSSHPublicKey(ne.StringValue(key.UUID)); err != nil {
