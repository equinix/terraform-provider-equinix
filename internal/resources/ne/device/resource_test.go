...
func TestNetworkDevice_updateResourceData(t *testing.T) {
	...
	assert.Equal(t, inputPrimary.Notifications, converters.ExpandSetToStringList(d.Get(neDeviceSchemaNames["Notifications"]).(*schema.Set)), "Notifications matches")
	...
	assert.Equal(t, inputPrimary.VendorConfiguration, converters.ExpandInterfaceMapToStringMap(d.Get(neDeviceSchemaNames["VendorConfiguration"]).(map[string]interface{})), "VendorConfiguration matches")
...
func TestNetworkDevice_expandSecondary(t *testing.T) {
	...
	expected := &ne.Device{
		...
		Notifications:       converters.ExpandSetToStringList(input[0].(map[string]interface{})[neDeviceSchemaNames["Notifications"]].(*schema.Set)),
