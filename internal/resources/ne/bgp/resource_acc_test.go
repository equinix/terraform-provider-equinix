...
func TestAccNetworkBGP_CSR1000V_Single_AWS(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: tfacc.NewTestAccConfig(context, withDevice, withSSHUser, withConnection, withBGP).Build(),
			...
			{
				Config: tfacc.NewTestAccConfig(contextWithChanges, withDevice, withSSHUser, withConnection, withBGP).Build(),
				Check: resource.ComposeTestCheckFunc(
					testAccNeBGPExists(resourceName, &bgpConfig),
...
func withBGP(t *tfacc.TestAccConfig) {
	t.Use(testAccNetworkBGP)
}
