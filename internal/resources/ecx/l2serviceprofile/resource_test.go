...
func TestFabricL2ServiceProfile_updateResourceData(t *testing.T) {
	...
	assert.Equal(t, input.OnBandwidthThresholdNotification, converters.ExpandSetToStringList(d.Get(ecxL2ServiceProfileSchemaNames["OnBandwidthThresholdNotification"]).(*schema.Set)), "OnBandwidthThresholdNotification matches")
	assert.Equal(t, input.OnProfileApprovalRejectNotification, converters.ExpandSetToStringList(d.Get(ecxL2ServiceProfileSchemaNames["OnProfileApprovalRejectNotification"]).(*schema.Set)), "OnProfileApprovalRejectNotification matches")
	assert.Equal(t, input.OnVcApprovalRejectionNotification, converters.ExpandSetToStringList(d.Get(ecxL2ServiceProfileSchemaNames["OnVcApprovalRejectionNotification"]).(*schema.Set)), "OnVcApprovalRejectionNotification matches")
	...
	assert.Equal(t, input.PrivateUserEmails, converters.ExpandSetToStringList(d.Get(ecxL2ServiceProfileSchemaNames["PrivateUserEmails"]).(*schema.Set)), "PrivateUserEmails matches")
