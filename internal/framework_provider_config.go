...
func (fp *FrameworkProvider) Configure(
	...
	var fpc FrameworkProviderConfig
	...
	resp.Diagnostics.Append(req.Config.Get(ctx, &fpc)...)
	...
	fpc.BaseURL = determineStrConfValue(
		fpc.BaseURL, config.EndpointEnvVar, config.DefaultBaseURL)

	fpc.ClientID = determineStrConfValue(
		fpc.ClientID, config.ClientIDEnvVar, "")

	fpc.ClientSecret = determineStrConfValue(
		fpc.ClientSecret, config.ClientSecretEnvVar, "")

	fpc.Token = determineStrConfValue(
		fpc.Token, config.ClientTokenEnvVar, "")

	fpc.AuthToken = determineStrConfValue(
		fpc.AuthToken, config.MetalAuthTokenEnvVar, "")

	fpc.RequestTimeout = determineIntConfValue(
		fpc.RequestTimeout, config.ClientTimeoutEnvVar, int64(config.DefaultTimeout), &resp.Diagnostics)

	fpc.MaxRetries = determineIntConfValue(
		fpc.MaxRetries, "", 10, &resp.Diagnostics)

	fpc.MaxRetryWaitSeconds = determineIntConfValue(
		fpc.MaxRetryWaitSeconds, "", 30, &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}

	oldStyleConfig := fpc.toOldStyleConfig()
