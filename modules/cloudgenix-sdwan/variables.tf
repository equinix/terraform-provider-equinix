...
variable "license_key" {
  ...
  type        = string
  validation {
    condition     = length(var.license_key) > 2
    error_message = "License key should have at least 2 characters."
  }
}
...
variable "license_secret" {
  ...
  type        = string
  validation {
    condition     = length(var.license_secret) > 2
    error_message = "License secret should have at least 2 characters."
  }
}
...
variable "secondary" {
  ...
  validation {
    condition     = ! try(var.secondary.enabled, false) || try(length(var.secondary.license_key) > 2, false)
    error_message = "Key license_key should have at least 2 characters."
  }
  validation {
    condition     = ! try(var.secondary.enabled, false) || try(length(var.secondary.license_secret) > 2, false)
    error_message = "Key license_secret should have at least 2 characters."
  }
}
