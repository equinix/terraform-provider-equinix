...
func TestAccECXL2PrivateServiceProfile(t *testing.T) {
	t.Parallel()
...
func testAccECXL2ServiceProfileAttributes(profile *ecx.L2ServiceProfile, ctx map[string]interface{}) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if v, ok := ctx["bandwidth_alert_threshold"]; ok && profile.AlertPercentage != v.(float64) {
			return fmt.Errorf("bandwidth_alert_threshold does not match %v - %v", profile.AlertPercentage, v)
		}
		if v, ok := ctx["speed_customization_allowed"]; ok && profile.AllowCustomSpeed != v.(bool) {
			return fmt.Errorf("speed_customization_allowed does not match %v - %v", profile.AllowCustomSpeed, v)
		}
		if v, ok := ctx["oversubscription_allowed"]; ok && profile.AllowOverSubscription != v.(bool) {
			return fmt.Errorf("oversubscription_allowed does not match %v - %v", profile.AllowOverSubscription, v)
		}
		if v, ok := ctx["api_integration"]; ok && profile.APIAvailable != v.(bool) {
			return fmt.Errorf("api_integration does not match %v - %v", profile.APIAvailable, v)
		}
		if v, ok := ctx["authkey_label"]; ok && profile.AuthKeyLabel != v.(string) {
			return fmt.Errorf("authkey_label does not match %v - %v", profile.AuthKeyLabel, v)
		}
		if v, ok := ctx["connection_name_label"]; ok && profile.ConnectionNameLabel != v.(string) {
			return fmt.Errorf("connection_name_label does not match %v - %v", profile.ConnectionNameLabel, v)
		}
		if v, ok := ctx["ctag_label"]; ok && profile.CTagLabel != v.(string) {
			return fmt.Errorf("ctag_label does not match %v - %v", profile.CTagLabel, v)
		}
		if v, ok := ctx["description"]; ok && profile.Description != v.(string) {
			return fmt.Errorf("description does not match %v - %v", profile.Description, v)
		}
		if v, ok := ctx["servicekey_autogenerated"]; ok && profile.EnableAutoGenerateServiceKey != v.(bool) {
			return fmt.Errorf("servicekey_autogenerated does not match %v - %v", profile.EnableAutoGenerateServiceKey, v)
		}
		if v, ok := ctx["equinix_managed_port_vlan"]; ok && profile.EquinixManagedPortAndVlan != v.(bool) {
			return fmt.Errorf("equinix_managed_port_vlan does not match %v - %v", profile.EquinixManagedPortAndVlan, v)
		}
		if v, ok := ctx["integration_id"]; ok && profile.IntegrationID != v.(string) {
			return fmt.Errorf("integration_id does not match %v - %v", profile.IntegrationID, v)
		}
		if v, ok := ctx["name"]; ok && profile.Name != v.(string) {
			return fmt.Errorf("name does not match %v - %v", profile.Name, v)
		}
		...
		if v, ok := ctx["oversubscription"]; ok && profile.OverSubscription != v.(string) {
			return fmt.Errorf("oversubscription does not match %v - %v", profile.OverSubscription, v)
		}
		if v, ok := ctx["private"]; ok && profile.Private != v.(bool) {
			return fmt.Errorf("private does not match %v - %v", profile.Private, v)
		}
		...
		if v, ok := ctx["redundancy_required"]; ok && profile.RequiredRedundancy != v.(bool) {
			return fmt.Errorf("redundancy_required does not match %v - %v", profile.RequiredRedundancy, v)
		}
		if v, ok := ctx["speed_from_api"]; ok && profile.SpeedFromAPI != v.(bool) {
			return fmt.Errorf("speed_from_api does not match %v - %v", profile.SpeedFromAPI, v)
		}
		if v, ok := ctx["tag_type"]; ok && profile.TagType != v.(string) {
			return fmt.Errorf("tag_type does not match %v - %v", profile.TagType, v)
		}
		if v, ok := ctx["secondary_vlan_from_primary"]; ok && profile.VlanSameAsPrimary != v.(bool) {
			return fmt.Errorf("secondary_vlan_from_primary does not match %v - %v", profile.VlanSameAsPrimary, v)
		}
		if v, ok := ctx["features_cloud_reach"]; ok && profile.Features.CloudReach != v.(bool) {
			return fmt.Errorf("features.cloud_reach does not match %v - %v", profile.Features.CloudReach, v)
		}
		if v, ok := ctx["features_test_profile"]; ok && profile.Features.TestProfile != v.(bool) {
			return fmt.Errorf("features.test_profile does not match %v - %v", profile.Features.TestProfile, v)
		}
