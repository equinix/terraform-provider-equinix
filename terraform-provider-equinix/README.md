...
* `percentage_alert` - *(Required)* specifies the port bandwidth threshold percentage. If the bandwidth limit is met or exceeded, an alert is sent to the seller
* `customspeed_allowed` - *(Required)* allow customer to enter a custom speed
...
* `api_available` - *(Required)* API integration allows you to complete connection provisioning in less than five minutes. Without API Integration, additional manual steps will be required and the provisioning will likely take longer
